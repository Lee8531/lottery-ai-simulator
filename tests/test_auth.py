...
from lottery_sim.auth import AuthStore, hash_password, verify_password
...
class AuthStoreTests(unittest.TestCase):
    ...
    def test_bootstrap_admin_creates_enabled_admin(self):
        with _temp_dir() as tmp:
            store = AuthStore(Path(tmp) / "app.sqlite3")

            user = store.bootstrap_admin("Admin", "secret")

            self.assertEqual(user.username, "admin")
            self.assertTrue(user.is_admin)
            self.assertTrue(user.is_enabled)
            self.assertIsNotNone(store.authenticate("admin", "secret"))
